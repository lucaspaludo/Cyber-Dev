...
while True:
    ...
    while deve_continuar: 
        ...
        if contador >= ITERACOES:
            caontador = 0
            tamAsteroides = random.randint(TAM_MINIMO, TAM_MAXIM0)
            posX = random.randint(0, LARGURA_JANELA - tamAsteroides)
            posY = -tamAsteroides
            velX = random.randint(-1, 1)
            velY = random.randint(VEL_MINIMA, VEL_MAXIMA)
            imagem_redimensionada = pygame.transform.scale(imagemASTEROIDES, (tamAsteroides, tamAsteroides))
            asteroides_rect = pygame.Rect(posX, posY, tamAsteroides, tamAsteroides)
        ...
        for asteroide in asteroides :
            moverElementos(asteroide)
            janela.blit(asteroide['imagem'], asteroide['objRect'])
        
        for asteroide in asteroides[:]:
            topo_asteroides = asteroide['objRect'].top
            if topo_asteroides > ALTURA_JANELA:
                asteroide.remove(asteroides)
    ...
    colocarTexto("pressione qualquer tecla para jogarnovamente", fonte, janela,(LARGURA_JANELA/10), (ALTURA_JANELA/2))
    pygame.display.update()
    aguardarEntrada()
    somFinal.stop()
