...
imagemTubarao = pygame.image.load('noob.png')
imagemPeixe = pygame.image.load('taco.png')
...
somComer = pygame.mixer.Sound('MM4.mp3')
pygame.mixer.music.load('MM5.mp3')
...
while deve_continuar:
    for evento in pygame.event.get():
        ...
        if evento.type == pygame.MOUSEBUTTONDOWN:
            peixes.append(
                {
                    'objRect': pygame.Rect(evento.pos[0], evento.pos[1], LARGURAPEIXE, ALTURAPEIXE),
                    'imagem': imagemPeixe,
                    'vel': VEL - 3
                }
            )
    ...
    if contador >= ITERACOES:
        ...
        peixes.append({
            'objRect': pygame.Rect(posX, posY, LARGURAPEIXE, ALTURAPEIXE),
            'imagem': imagemPeixe,
            'vel': velRandom
        })
